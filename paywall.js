...
(function() {
    ...
    var PREMIUM_CARDS = ['familyCard','healthCard','ganzhiRelationsCard','yunweiCard','dayunCard','liunianCard','liuyueCard','plainLanguageCard','combinationCard','gejuCard','careerCard'];
    ...
    function showRefund() {
        ...
        m.innerHTML='<div style="background:#1a1a2e;border-radius:16px;padding:25px;max-width:400px;width:100%;color:#fff;"><h3 style="margin:0 0 15px;">📋 申请退款</h3><input id="rOrderId" placeholder="订单号（选填）" style="width:100%;padding:10px;margin-bottom:10px;background:#2a2a3e;border:1px solid #444;border-radius:8px;color:#fff;box-sizing:border-box;"><textarea id="rReason" placeholder="退款原因 *" style="width:100%;padding:10px;margin-bottom:10px;background:#2a2a3e;border:1px solid #444;border-radius:8px;color:#fff;min-height:60px;box-sizing:border-box;"></textarea><input id="rContact" placeholder="联系方式 *（微信号或手机号）" style="width:100%;padding:10px;margin-bottom:15px;background:#2a2a3e;border:1px solid #444;border-radius:8px;color:#fff;box-sizing:border-box;"><button id="rSubmitBtn" style="width:100%;padding:12px;background:linear-gradient(135deg,#4a1a8a,#6b21a8);color:#ffd700;border:none;border-radius:8px;font-size:16px;cursor:pointer;">提交退款申请</button><p style="font-size:11px;color:#666;margin-top:8px;text-align:center;">提交后将在24小时内处理</p></div>';
        document.body.appendChild(m);
        tap(document.getElementById('rSubmitBtn'), function(){
            var oid=document.getElementById('rOrderId').value.trim(), reason=document.getElementById('rReason').value.trim(), contact=document.getElementById('rContact').value.trim();
            if(!reason){toast('请填写退款原因');return;}if(!contact){toast('请填写联系方式');return;}
            fetch(API+'/refund/apply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({order_id:oid,reason:reason,contact:contact})}).then(function(r){return r.json();}).then(function(d){if(d.success){toast('✅ 退款申请已提交');closeRefund();}else toast(d.error||'提交失败');}).catch(function(){toast('网络错误');});
        });
